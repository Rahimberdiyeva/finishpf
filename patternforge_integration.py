...
def ensure_uv(obj):
    ...
    if not mesh.uv_layers:
        # Сохраняем активный объект и режим
...
def setup_pbr_material(mat, textures):
    ...
    # AO смешиваем с BaseColor через Multiply
    if textures.get("ao") and textures.get("basecolor"):
        ...
        if ao_img and base_node:
            ...
            links.new(base_node.outputs["Color"], mix.inputs[1])  # Color1
            links.new(ao_node.outputs["Color"], mix.inputs[2])   # Color2
            links.new(mix.outputs["Color"], principled.inputs["Base Color"])
            # Отключаем прямую связь base_color, если была
...
def import_zip(zip_path, context):
    ...
    # Используем временную папку с гарантированным удалением
    ...
    try:
        ...
        # Проверяем, что есть хотя бы basecolor, иначе бессмысленно
        if not textures["basecolor"]:
            raise ValueError("В ZIP-архиве не найдена карта Base Color (albedo/diffuse)")
        ...
        # Безопасно удаляем старый материал
        old_mat = bpy.data.materials.get(mat_name)
        if old_mat and old_mat.users == 0:
            bpy.data.materials.remove(old_mat)
        elif old_mat:
            log(f"Material {mat_name} is still in use, creating new one with unique name")
