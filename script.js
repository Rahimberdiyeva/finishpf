...
const fragmentShader = `
  ...
  float compute2DPattern(vec2 uv) {
    ...
    if(uPatternType == 0) {
      ...
      return clamp((w1 + w2) * 0.6 + 0.5, 0.0, 1.0) * uIntensity;
    }
    else if(uPatternType == 1) {
      float v = worley(st * 3.5);
      v = pow(v * 1.2, 0.8);
      return clamp(v * uIntensity, 0.0, 1.0);
    }
  ...
  void main() {
    ...
    if (uTestMode == 1) { float r = 1.0 - pow(patternValue, uRoughnessContrast); r = clamp(r, 0.04, 0.96); gl_FragColor = vec4(r, r, r, 1.0); return; }
    if (uTestMode == 2) { float m = clamp((patternValue - uMetalThreshold) * uMetalScale, 0.0, 1.0); gl_FragColor = vec4(m, m, m, 1.0); return; }
...
document.getElementById('exportModelBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (customModel) {
      ...
      exportScene.add(new THREE.Mesh(geom, mat));
    }
...
function cloneUniforms(src) {
  const dst = {};
  for (const key in src) dst[key] = { value: src[key].value };
  return dst;
}
...
patternBar?.addEventListener('touchstart', e => {
